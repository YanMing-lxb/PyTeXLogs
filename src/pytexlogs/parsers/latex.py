...
latex_warn_re = re.compile(
    r"^((?:(?:Class|Package|Module) \S+)|LaTeX(?: \S*)?|LaTeX3) "
    r"(Warning|Info):\s+(.*?)(?: on(?: input)? line (\d+))?(\.|\?)?$"
)
...
_LATEX_EXT = (
    r"tex|sty|cls|aux|fd|cfg|def|ldf|clo|bbl|blg|idx|ind|gls|glo|acr|nls|nlo|"
    r"toc|lof|lot|out|log|ist|alg|slg|dvi|pdf|ps|eps"
)
...
rerun_to_get_citations_re = re.compile(
    r"^(?:Package (\S+) Warning: Please \(re\)run Biber to get citations right\.?|"
    r"LaTeX Warning: Citation\(s\) may have changed\. Rerun to get citations correct\.?)$"
)


class _TempEntry(dict[str, Any]):
    pass
...
class LatexLogParser(BaseLogParser):
    ...
    def _parse_line(self, line: str) -> None:
        ...
        if undef_match:
            ref_type, label, line_num = undef_match.groups()  # type: ignore[assignment]
            if self.current_result and self.current_result.get("type") is not None:
                self._flush_current()
        ...
        if warn_match:
            ...
            line_num = warn_match.group(4)  # type: ignore[assignment]
        ...
        if extra_match:
            cls_name, msg, line_num = extra_match.groups()  # type: ignore[assignment]
            if self.current_result and self.current_result.get("type") is not None:
                self._flush_current()
            self.current_result = _TempEntry(
                type=LogLevel.WARNING,
        ...
        if extra_match:
            pkg_name, msg, line_num = extra_match.groups()  # type: ignore[assignment]
            if self.current_result and self.current_result.get("type") is not None:
                self._flush_current()
            self.current_result = _TempEntry(
                type=LogLevel.WARNING,
        ...
        if extra_match:
            msg, line_num = extra_match.groups()  # type: ignore[assignment]
            if self.current_result and self.current_result.get("type") is not None:
                self._flush_current()
            self.current_result = _TempEntry(
                type=LogLevel.WARNING,
