...
try:  # rich 是可选依赖，PyTeXLogs 独立库默认不安装（NFR-1 零 runtime 第三方依赖）
    from rich.console import Console as _Console
    _RICH_AVAILABLE = True
except Exception:  # noqa: BLE001 - 用户环境没装 rich 时 fallback 到标准库
...
_RICH_CONSOLE: object = None  # type: ignore[no-redef,assignment]
if _RICH_AVAILABLE:
    _RICH_CONSOLE = _Console(highlight=False, markup=True, soft_wrap=False)  # type: ignore[misc]
else:
    # 无 rich 时：提供一个兼容 Console.print(..., style=...) 语义的最小桩
    class _FallbackConsole:
        """PyTeXLogs 无 rich 时的最小兼容桩：只接受 print() 参数的子集，忽略 markup/style。"""
